...
const ScrapSearched = (props) => {

    const { state } = useLocation();
    const [bigram, setBigram] = useState(Array());
    const [scraps, setScraps] = useState(Array());
    /* ↓重複データ削除後のユニークな結果 */
    const [uniqueResults, setUniqueResults] = useState(Array());
    /* ↓Firestoreから直接表示させる */
    ...
    /* ↓bigramの処理が終わったときに検索結果と合致するものをデータベースから抜き出す */
    useEffect(() => {
        if (bigram.length)
        ...
        onSnapshot(scrapData, (scrap) => {
            scrap.docs.map((doc) => (
                bigram.forEach(word => {
                    if(doc.data().tokenMap.includes(word) || doc.data().languages.includes(state) || doc.data().others.includes(state)) {
    ...
    /* ↓重複しているデータを削除してユニークな状態に変換する */
    useEffect(() => {
        const temp = Array.from(new Map(scraps.map((scrap) => [scrap.id, scrap])).values());
        setUniqueResults(temp);
    }, [scraps])
                    ...
                    <div className='displayQuestions'>
                        <div className='flexbox'>
                            ...
                            <div className='Scraps'>
                                ...
                                <p className='text-center'>検索結果: {uniqueResults.length}件</p>
                                {/* データベースに登録された質問内容を一つ一つ表示(ForEach構文に似たmap関数を使用) */}
                                {uniqueResults.length ? (
                                    uniqueResults.map((scraps) => (
                                    ...
                                    <div className='noResults-wrapper'>
                                        <h5>検索結果に該当するデータがありません。</h5>
                                        <h5>キーワードを変更して再度検索をしてみてください。</h5>
                                    </div>
                                )
                                }
                            </div>
